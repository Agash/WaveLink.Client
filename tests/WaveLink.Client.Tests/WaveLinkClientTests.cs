...
{
    private static WaveLinkClientOptions OptionsFor(FakeWaveLinkServer server) =>
        new()
        {
    ...
    private static string MethodOf(JsonElement request) =>
        request.TryGetProperty("method", out JsonElement method)
    ...
    public async Task ConnectAsync_WhenServerIsNotWaveLink_Fails()
    {
        ...
        await using FakeWaveLinkServer server = FakeWaveLinkServer.Start(request =>
            FakeWaveLinkServer.ApplicationInfoReply(IdOf(request), appId: "SOMETHING-ELSE")
        );
        await using WaveLinkClient client = new(OptionsFor(server));
    ...
    {
        await using FakeWaveLinkServer server = FakeWaveLinkServer.Start(request =>
            FakeWaveLinkServer.ApplicationInfoReply(IdOf(request), interfaceRevision: 0)
        );
        await using WaveLinkClient client = new(OptionsFor(server));
        ...
        Assert.IsTrue(
            ex.Message.Contains("interfaceRevision", StringComparison.Ordinal),
            ex.Message
        );
    }
    ...
    public async Task CallAsync_WhenServerRepliesOutOfOrder_MatchesEachResultToItsRequest()
    {
        ...
        TaskCompletionSource<string> deferred = new(
        ...
        Task<InputDevicesResult> first = client.GetInputDevicesAsync(
            TestContext.CancellationTokenSource.Token
        );
        InputDevicesResult second = await client.GetInputDevicesAsync(
            TestContext.CancellationTokenSource.Token
        );
    ...
    {
        await using FakeWaveLinkServer server = FakeWaveLinkServer.Start(request =>
            MethodOf(request) == "getApplicationInfo"
                ? FakeWaveLinkServer.ApplicationInfoReply(IdOf(request))
                : null
        );
    ...
    public async Task Notification_WhenServerPushesLevelMeters_RaisesTheEvent()
    {
        ...
        TaskCompletionSource<LevelMeterChangedParams> raised = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        ...
        LevelMeterChangedParams received = await raised.Task.WaitAsync(
    ...
    public async Task Notification_WhenServerPushesInputDevices_UpdatesTheCachedState()
    {
        ...
        TaskCompletionSource<IReadOnlyList<InputDevice>> raised = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        ...
        IReadOnlyList<InputDevice> devices = await raised.Task.WaitAsync(
            TimeSpan.FromSeconds(10),
            TestContext.CancellationTokenSource.Token
        );

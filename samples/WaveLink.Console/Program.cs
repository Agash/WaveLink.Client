...
client.FocusedAppChanged += (_, e) =>
    Console.WriteLine($"Focused app: {e.Name} ({e.Id}) -> channel {e.Channel?.Id}");
client.LevelMeterChanged += (_, e) =>
    Console.WriteLine($"Level meters update: channels={e.Channels?.Count ?? 0}");
...
await client.SetSubscriptionAsync(
    new SetSubscriptionParams
    {
        FocusedAppChanged = new SubscriptionToggle { IsEnabled = true },
        LevelMeterChanged = new LevelMeterSubscription
        {
            IsEnabled = true,
            Type = "channel",
            Id = "all",
